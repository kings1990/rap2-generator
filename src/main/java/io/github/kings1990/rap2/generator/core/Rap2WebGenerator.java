...
public class Rap2WebGenerator {
    ...
    private static final String FIELD_EXP = "(\\s*|\\t*)(private|protected|public)\\s+(?!static|static\\s+final)(.*)\\s+(\\w+);$";
    ...
    public String generator(JSONArray all) throws IOException {
        ParseWebConfig parseConfig = getParseWebConfig();
        if (parseConfig == null) {
            return "【error】配置文件解析异常";
        }
        //配置开始
    ...
    public JSONArray parse() throws Exception {
        ...
        if (StringUtils.isNotBlank(requestJavaClassname)) {
            parseJava(jsonArray, REQUEST, parentId, javaDirPath,requestJavaClassname, null);
        }
        ...
        String jsonString = parseConfig.getResponseConfigJson();
        ...
        for (ResponseTemplateConfig responseConfig : responseConfigList) {
            if (responseConfig.isResultFlag()) {
                if (!"Object".equals(dealResponseType)) {
                    ...
                    JSONObject jsonObjectResult = dealCommonParam(responseConfig.getProperty(), responseConfig.getDescription() + "(" + dealResponseType + ":" + responseResultData.getDescription() + ")", responseResultType.getExp(), "-1", "response");
                    jsonArray.add(jsonObjectResult);
                } else {
                    //解析内部对象
                    JSONObject jsonObjectResult = dealCommonParam(responseConfig.getProperty(), responseConfig.getDescription(), responseResultType.getExp(), "-1", "response");
                    jsonArray.add(jsonObjectResult);
                }
            } else {
                JSONObject jsonObjectMsg = dealCommonParam(responseConfig.getProperty(), responseConfig.getDescription(), responseConfig.getType().getExp(), "-1", "response");
        ...
        if(StringUtils.isNotBlank(responseJavaClassname) || StringUtils.isNotBlank(responseResultData.getDescription())) {
            JSONArray resultResponse = parseJava(jsonArray, RESPONSE, parentId, javaDirPath, responseJavaClassname, responseResultData);
    ...
    private JSONObject dealCommonParam(String fieldString, String annotationString, String type, String parentId,  String scope) {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("name", fieldString);
        jsonObject.put("description", annotationString);
        String dealType = dealType(type);
